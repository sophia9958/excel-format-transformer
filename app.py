...
def parse_time_logic(val):
    """处理 [h]:mm:ss 的特殊累计时间格式，无视24小时进位限制"""
...
✅ **多 Sheet 智能寻呼**：表 A 与表 B 同名工作表自动路由、精准对齐。
---
📬 **联系售后**：若遇报错，请点击底部【排错专区】一键复制日志并发送至 **nolinda@126.com**。
""")
...
custom_defaults_text = st.sidebar.text_area("1. 默认填充 (字段=内容)", placeholder="更新日期=260712", height=100)
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            for s_name in xls_tpl.sheet_names:
                ...
                if unnamed:
                    st.warning(f"🚨 **表 A ({target_raw_sheet if is_raw_excel else 'CSV文件'}) 发现无名列！** 请根据内容在左侧手动指定：")
        ...
        st.success("🎉 全新多 Sheet 模式数据清洗匹配成功！")
        st.download_button("📥 下载交付表", data=output.getvalue(), file_name=f"Result_{datetime.datetime.now().strftime('%y%m%d')}.xlsx")
        
    except Exception as e:
        st.error("❌ 发生了预期外的处理错误！")
        with st.expander("🛠️ 售后一键排错专区 (请展开并复制代码发送给 nolinda@126.com)"):
            import traceback
            err_log = traceback.format_exc()
            st.text_area("错误日志", value=err_log, height=200)
