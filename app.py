...
---
系统会自动把 **【表A（原始数据）】** 里的数据提取/筛选并填入 **【表b（带有你需要格式、字段/表头的模版）】** 里。
* **自定义填充：** 如果A表没有B表某字段，你可以为这些字段设置默认内容（详见左侧第1点）。
* **无表头预警：** 如果A表某列无表头但该列有内容，传完表A后见下方提示，可在左侧第2点手动对号修正。
* **求助与售后：** 遇到严重报错，请点击底部【排错专区】的复制按钮，将日志发给 **nolinda@126.com**。
""")
...
with col_u1: raw_file = st.file_uploader("📂 上传【表A：数据源】(只读第1个Sheet)", type=["csv", "xlsx", "xls"])
with col_u2: template_file = st.file_uploader("📋 上传【表B：目标模板】(支持多Sheet精准定位)", type=["xlsx", "xls"])
...
if raw_file:
    try:
        ...
        else: df_raw = pd.read_excel(raw_file, dtype=str).fillna("") # 强行只读表A第一个Sheet
        ...
        if unnamed:
            ...
            for idx, pre in unnamed: st.write(f"👉 表A 第 `{idx}` 列 ➔ 内容预览: `{pre}`")
...
st.sidebar.markdown("---")
st.sidebar.header("🔍 3. 模板精准定位")

target_sheet = None
if template_file:
    xls_tpl = pd.ExcelFile(template_file)
    sheet_names = xls_tpl.sheet_names
    
    # 【改动核心 1】：下拉菜单选择要操作的 Sheet
    if len(sheet_names) > 1:
        target_sheet = st.sidebar.selectbox("🎯 (1) 选择你要填入数据的【目标 Sheet】：", sheet_names)
    else:
        target_sheet = sheet_names[0]
        st.sidebar.info(f"👉 表B仅有1个Sheet: {target_sheet}")
    
    # 【改动核心 2】：明确指定选中 Sheet 的表头在第几行
    f_h = st.sidebar.number_input(
        f"🎯 (2) 『{target_sheet}』的表头在第几行？(0为自动)", 
        min_value=0, max_value=20, value=0
    )
    force_header_config[target_sheet] = f_h
    
    st.sidebar.markdown("---")
    st.sidebar.subheader("⚙️ 高级特征词设置")
    with st.sidebar.expander("点击展开设置特征词"):
        header_keywords_input = st.text_input("表头定位特征词", value="编码,名称,日期,类型,状态,标题,时间,片单,导演,演员,ID")
        time_keywords_input = st.text_input("时长识别特征词", value="总,时长,时间,片长,总长,Duration")
    ...
    # 仅针对当前选中的 target_sheet 提取时间列
    all_time_cols_in_template = []
    try:
        df_preview = pd.read_excel(xls_tpl, sheet_name=target_sheet, header=None, nrows=10)
        h_idx_preview, hs = (f_h-1, [str(x).strip() for x in df_preview.iloc[f_h-1].values if pd.notna(x)]) if f_h > 0 else find_headers(df_preview, custom_header_keywords)
        all_time_cols_in_template.extend([h for h in hs if is_duration_col(h, custom_time_keywords)])
    except: pass
    
    selected_time_cols = st.sidebar.multiselect("以下字段将执行 [h]:mm:ss 转换", list(set(all_time_cols_in_template)), default=list(set(all_time_cols_in_template)))
else:
    st.sidebar.caption("⏳ 上传【表B】后，即可在此进行模板定位。")

# ==================== 9. 执行核心映射与渲染看板 ====================
if df_raw is not None and template_file and target_sheet:
    output = BytesIO()
    final_reports = {}
    debug_log = {"诊断": {}}

    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # 【改动核心 3】：完全抛弃循环，只处理下拉框里选中的那一个 target_sheet
        df_tpl_meta = pd.read_excel(xls_tpl, sheet_name=target_sheet, header=None, nrows=10)
        
        if not df_tpl_meta.empty:
            f_h = force_header_config.get(target_sheet, 0)
            ...
            final_reports[target_sheet] = report
            debug_log["诊断"][target_sheet] = {"识别行": h_idx+1, "表B字段": headers}

    st.markdown("---")
    st.subheader(f"📊 数据对齐与填充看板 (表A ➡️ 表B 『{target_sheet}』)")

    for s_name, report in final_reports.items():
        ok_count = sum(1 for x in report if x['s'] == 'ok')
        with st.expander(f"📁 『{s_name}』 预览 (提取成功: {ok_count} | 填充/留空: {len(report)-ok_count})", expanded=True):
            c1, c2 = st.columns(2)
            with c1:
                st.write("🟢 **a表提取成功**")
    ...
    st.success("🎉 处理完成！")
    st.download_button("📥 下载专属数据交付表", data=output.getvalue(), file_name=f"{target_sheet}_{today_str}.xlsx")
    ...
    with st.expander("🛠️ 排错专区 (遇到问题请复制此段)"):
        ...
        <div style="text-align: right; margin-bottom: -15px;">
            <button id="copy-btn" style="background-color: #ff4b4b; color: white; border: none; padding: 6px 14px; font-size: 13px; border-radius: 4px; cursor: pointer; font-family: inherit; font-weight: 500;">📋 一键复制全部错误信息</button>
        </div>
        <script>
        document.getElementById('copy-btn').addEventListener('click', function() {{
            navigator.clipboard.writeText(decodeURIComponent(atob('{b64_json}').split('').map(function(c) {{ return '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2); }}).join(''))).then(function() {{
                alert('复制成功！快发给 nolinda@126.com 吧~');
            }});
        }});
        </script>
        """
